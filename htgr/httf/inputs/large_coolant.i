...
m_dot_in = ${fparse (m_dot_total*(A_l/A_total))/n_large_channels} # kg/s
R_He = 2076.9 # J/kg-K
...
[Functions]
  ...
  [system_pressure]
    ...
    format = columns
  []
  # Creates a function which calculates the inlet mass flowrate of a single large flow channel
  [mdot_in_fn]
    type = ParsedFunction
    expression = '${fparse (m_dot_total*(A_l/A_total))/n_large_channels}'
    symbol_names = 'm_dot_total A_l A_total n_large_channels'
    symbol_values = '${m_dot_total} ${A_l} ${A_total} ${n_large_channels}'
  []
[]
...
[Components]
  # Inlet which initializes the large flow channel based on an inlet temperature and mass flow rate
  [channel_inlet]
