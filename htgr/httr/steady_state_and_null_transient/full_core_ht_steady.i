# ==============================================================================
# Full-core homogenized heat transfer model (steady-state)
...
[ICs]
  [Tsolid_ic]
    type = FunctionIC
    block = '${full_fuel_blocks} ${full_cr_blocks} ${full_pr_blocks}'
    variable = Tsolid
    function = Tsolid_init_func # so that RELAP gets a reasonable initial value
  []
  [Tfluid_ic]
    type = FunctionIC
    block = '${full_fuel_blocks} ${full_cr_blocks} ${full_pr_blocks}'
...
[AuxVariables]
  [power_density]
    block = '${fuel_blocks}'
    initial_condition = 0.86283835e6
    order = FIRST
    family = L2_LAGRANGE
  []
...
[UserObjects]
  ...
  [average_axial_temperature_UO] # for postprocessing only (e.g. to get an idea of the axial average temperature)
    ...
    variable = Tsolid
    direction = x
    # bounds = '0 0.29 0.58 0.87 1.16 1.45 1.74 2.03 2.32 2.61 2.9 3.19 3.48 3.77 4.06 4.35 4.64 4.93 5.22' # for 18 axial elements
...
[MultiApps]
  [bison]
    ...
    input_files = 'fuel_elem_steady.i'
