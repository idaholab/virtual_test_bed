...
# - 2D RZ geometry
# - FV using the new FV physics syntax
# ==============================================================================
# MODEL PARAMETERS
# ==============================================================================
# Problem Parameters -----------------------------------------------------------
...
[FVBCs]
  # Note: applying the radiation separately is not best practice
  # because the surface temperature used here does not take into account
...
# ==============================================================================
# Constant (q,v,mu,p,T) conditions as we hold the null transient
# ==============================================================================
...
[Functions]
  ...
  [mu_ramp_fn]
    ...
    y = '1  1 1'
  []
  # MFR is constant, ramp-up has been performed in steady-state inputs
  [mfr_fn]
    type = PiecewiseLinear
    x = '    0      13'
    y = ' ${mfr}    ${mfr}'
  []
  # Pressure is constant in null transient
  [pressure_out_fn]
    type = PiecewiseLinear
    x = '  0           13'
    y = '${p_outlet}  ${p_outlet}'
  []
  # Allows for fast growth in time step size
  [dt_max_fn]
    type = PiecewiseLinear
    x = '-10 0 16 30 1000 5000 20000 100000 200000 500000'
    y = '  1 1 1  20  100   200  400   1000   1000   1000'
  []
[]
...
[Outputs]
  csv = true
  exodus = true
  # Remove two postprocessors created by the action to control verbosity
  [console]
    type = Console
