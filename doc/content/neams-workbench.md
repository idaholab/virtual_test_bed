...
<!--A portable bash script ([PBS](https://en.wikipedia.org/wiki/Portable_Batch_System)) scheduler is available on Sawtooth and PBS directives can be updated if needed (number of nodes, cores, wall time, etc ..) or left to their default values (one node, one cpu and a wall time of ten minutes). To edit the PBS directive, click on the back downward arrow right to the `Run` buttom and select `Customize Run Options...`. A window should open to show the PBS directives after scrowling down. Edit the entries to your convenience, click `Apply` and then `Ok` to close the widget as shown in [Figure 11](fig:hpc_on_demand_pbs_options). To submit the job to the queue managed by the PBS scheduler, click `Run`.-->
...
### Step 4: visualize the numerical solution from the NEAMS Workbench GUI
  ...
  !media neams_workbench/hpc_on_demand_render_view.png style=width:80% id=fig:hpc_on_demand_render_view caption=Figure 12: Drag and drop the Exodus solution output file *griffin_out.e* in the NEAMS Workbench GUI to open a visualization tab or render view.
