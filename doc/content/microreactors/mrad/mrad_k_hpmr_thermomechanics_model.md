...
In the Sockeye heat pipe model, heat pipe operational limits are not enforced in the current model, representing a simplifying assumption that the heat pipes operate within their design limits throughout the transient. This assumption is considered acceptable for the load following scenario examined here, where the heat removal reduction is driven by the secondary-loop boundary condition rather than by heat pipe failure.
