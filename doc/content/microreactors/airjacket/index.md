...
pressure and energy balance equations, which is the primary source of improvement in the results obtained using
generic correlations. These equations in the workflow described below
can be replaced by systems-levels tools or other software that can be used in conjunction with [!ac](CFD) in the
...
which has been extensively validated and verified, and used for nuclear engineering applications.
...
our analysis. We discuss why we are using Nek5000 in the first place, followed by a look at the
...
which lowers floating-point operations and storage requirements for SEM significantly. For example, in 3D, the action of discrete operators can be
...
This, however, limits the method to quadrilateral or hexahedral mesh elements, as 1D matrices need to be along clearly separable orthogonal axes for matrix-
free forward operator evaluation.
...
of an order of at least 5, and ideally 7-11. Accordingly, if your prior experience is meshing for conventional finite-elements,
consider making a coarser mesh than usual and run the simulation at a high-order, otherwise you are likely to end up with an over-resolved mesh,
...
\label{eq:ins}
   \tilde{\nabla} \cdot \tilde{u} = 0 \\
    ...
    \frac{\partial T}{\partial \tilde{t}} + \tilde{u} \cdot \tilde{\nabla} \theta = \frac{1}{Pe} \tilde{\nabla}^2 \theta
\end{equation}
...
The air jacket is a natural circulation driven passive heat removal component of a high-temperature gas-cooled microreactor.
...
The inner surface of the air gap is heated by the reactor vessel, whereas the outer surface is assumed insulated.
The air inlet at the bottom supplies air at 50$^\circ$C. The heated section, which is held at a constant temperature
...
temperature Dirichlet condition which has a value equivalent to the maximum safe temperature of the hot wall, and are asked to investigate the flow
characteristics, friction factor, and heat transfer coefficient at said limiting condition.

However, the exact inlet boundary condition or the mass flow rate, and the outlet temperature are not known. However, based
...
direction of the wedge-shaped slice of the circular air jacket domain. Hence a 2D simulation is adequate for the purposes
of calculation flow and heat transfer characteristics for this geometry when using RANS. The problem is simulated in the axisymmetric
mode to capture the cylindrical nature of the 3D geometry in 2D. The details of the axisymmetric mode setup are in the next section of
the tutorial.
...
Given temperature boundary conditions and dimensions, our main objective was to determine the friction factor and the Nusselt number ($Nu$). The pressure balance is given by:
...
\end{equation}
where the mean internal gravitational pressure drop and minor losses are modeled respectively as
\begin{equation}
...
\end{equation}
where the subscript $0$ implies a reference value. 
Combining these with the total pressure balance given by [eq:pbalance] yields
\begin{equation}
    \frac{1}{2} \frac{f}{D} H \rho_0 U_m ^2+\rho_m g H =\rho_{\infty} g H
\end{equation}
\begin{equation}\label{eq:iterative_re}
    \frac{1}{2} \frac{f}{D} H \rho_0 U_m ^2 =\left(\rho_{\infty}-\rho_m\right) g H
\end{equation}

Since the gravitational pressure drop is a hydrostatic quantity, we adopt the ``mean density'' to imply volumetric mean (simple volumetric average) rather than bulk mean (velocity-weighted volumetric average).
...
\begin{equation}
    \rho_{\infty}-\rho_m = \rho_0\beta_0\left(T_m-T_{in}\right) .
\end{equation}
...
The pressure balance is based on Bernoulli's equation. Assuming a pipe of hydraulic diameter $D_h$ and height $H$, we can write (neglecting form losses):
\begin{equation}
\Delta P_{internal} = \Delta P_{external}
\end{equation}
\begin{equation}
(\rho _{ambient} - \rho _{mean} ) gH = \Delta P_{friction} = \frac{1}{2} \frac{f_D \rho_{mean} \bar{U}^2 H}{D_h} \label{maineq1} \\
\end{equation}
where $\rho_{ambient}$ is theambient density. We will also be assuming that ambient temperature and density are the same as inlet temperature and density respectively.

The energy balance is given by:
\begin{equation}
q_{hot wall} = q_{convection}
\end{equation}
\begin{equation}
A_{hw} h(T_{hot} - T_{mean}) = \dot{m} C_p (T_o - T_{in})
\end{equation}
\begin{equation}
A_{hw} h(T_{hot} - T_{mean}) = \rho_{mean} \bar{U} A_c C_p (T_o - T_{in}) \label{maineq2}
\end{equation}
where $A_{hw}$ is the hot wall area, $T_{hot}$ the hot wall temperature, $A_c$  the cross section area of the channel.

The main equations to be solved for $\bar{U}$ and $T_o$ are [maineq1] and [maineq2]. These are non-linear, and can be easily solved with symbolic mathematics software or iterative solvers
packages such as `scipy` (see the provided `re_estimate.py` for an example).

Approximations for for $T_m$ (which require an approximate heat transfer coefficient $h$) and $f$ using conventional correlations (such as Dittus-Boelter and Blasius correlations, respectively) can be used for an initial guess, but as the simulation is run, values are calculated directly from the results. With these results, the correct $f$ and $Nu$ can be determined for a given $U_m$.
...
Inlet conditions were generated using a small mesh extruded from the inlet ([inletbc_mesh]), to which periodic boundary conditions
were applied in the streamwise direction. Fully-developed results from this mesh were interpolated to the main mesh to create the inlet conditions for velocity.
For the outlet, we used a do-nothing boundary condition.
...
Wall resolution was ensured by placing all boundary layer points within $y^+\leq$1.
...
\begin{equation}
\tilde{F} = \frac{ \beta_0 (T_{wall} - T_0) g L}{U ^2} \hat{z} = Ri \: \theta
\end{equation}
where $Ri$ is the Richardson number, $T_{wall}$ the dimensional hot-wall temperature, and $\theta$ the non-dimensional local temperature.
...
and compare it to the expected pressure drop based on [eqn:dp_exp], that is, zero. If the two pressure drops do not match, it is possible to iterate on the value
...
The hot wall temperature is set to a constant value of 1 in nondimensional terms (420 $^\circ$C), that is implemented with a short, linear ramp
to avoid sharp gradients that create problems with resolution of scalars. The walls have zero Dirichlet conditions for $k$ and $\tau$. The outlet
...
by every MPI process, and attempting an operation like a global sum can cause the program to freeze. The Boussinesq term is gradually
...
When meshing, we performed h-refinement by adding elements to areas of transition between the channel and the plena, and other
regions where under-resolution was observed. Then the final mesh refinement was performed using p-refinement, as is typical
...
To iterate on the Reynolds number until the correct pressure drop of nearly zero (less than the pressure solver tolerance) is obtained,
...
The excess upper
plenum outlet region has been clipped by about 50% to remove the region that exists for the sole purpose of allowing the flow
...
The quantities of interest calculated from steady state results are summarized in [tab_results]. They
are calculated from when the simulation achieves a steady state i.e. the fluctuation in these metrics
is below the solver tolerance. Note that CFD also allows the estimation of certain data that cannot
be reliably estimated from correlations alone without signficant approximations.

!table id=tab_results caption=Summary of results.
| Parameter  | Correltation-based estimate  | CFD-based result  | Units |
| :- | :- | :- | :- |
