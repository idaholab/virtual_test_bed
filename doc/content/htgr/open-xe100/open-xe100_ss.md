...
Each number in the block ID fields corresponds to a matching element of the
...
Sub-blocks for each material (fuel, reflector, cavity) are declared here. Each
is of type `CoupledFeedbackMatIDNeutronicsMaterial` which uses coupled variables
to interpolate tabulated cross sections provided in the ISOXML file. The
sub-blocks have the following additional parameters:
...
a power density auxiliary variable such that its integral of the domain is equal to
the user-specified power. This object takes the following parameters:
