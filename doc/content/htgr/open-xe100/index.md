...
!tag name=Griffin Open Xe-100 Model
     ...
     image=https://mooseframework.inl.gov/virtual_test_bed/media/open-xe100/*.png
...
it only includes a 2D axisymmetric representation of the reactor core.
This model is discussed in detail in Stewart et. al. [!citep](Stewart2022).
The instaneous core power density shown in [iqs_core_power_density] can differentiate
...
!media open-xe100/iqs_core_power_density.png
       ...
       caption=Instaneous core power density of the Open Xe-100 Reactor with the improved quasi-static (IQS) method.

Griffin solves the neutron transport problem using the diffusion equation
approximation. In solving the diffusion equation, Griffin can use methods like
point reactor kinetics (PRKs) or the improved quasi-static (IQS) method. For
this model, there are four input files: (1) a steady-state eigenvalue calculation,
...
An explanation of the inputs follows:
