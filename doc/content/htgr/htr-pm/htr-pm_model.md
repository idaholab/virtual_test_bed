...
The transient PLOFC simulation is a restart from the steady-state simulation with some minor modifications in the input files. The `[Problem]` block in all three input files are modified where the path of the restart file is specified using the `restart_file_base` parameter. The `LATEST` keyword is used to specify the latest checkpoint as the restart time stamp. Alternatively, the restart time stamp  can be set to another time step by specifying the time-step number.

!listing htgr/htr-pm/ss-main-transient.i block=Problem language=cpp

!listing htgr/htr-pm/ss-primary-loop-transient.i block=Problem language=cpp

!listing htgr/htr-pm/ss-rccs-water-transient.i block=Problem language=cpp

The SubApp files in the `[MultiApps]` block are changed to the transient files

!listing htgr/htr-pm/ss-main-transient.i block=MultiApps language=cpp
...
!listing htgr/htr-pm/ss-primary-loop-transient.i block=inlet outlet language=cpp
