...
[Mesh]
  ...
  []
  # Define equivalent fully-homogeneous core from input heterogeneous RGMB core
  [full_hom_core]
    type = EquivalentCoreMeshGenerator
    input = het_core
    target_geometry = full_hom
    quad_center_elements = true
    max_axial_mesh_size = 10
    debug_equivalent_core = true
  []
  # Apply mesh refinement to output homogeneous mesh
  [fine_mesh]
    ...
    refinement = 1
  []
  # Define and apply coarse mesh to the refined mesh based on the un-refined homogeneous mesh
  [coarse_mesh]
    type = CoarseMeshExtraElementIDGenerator
    input = fine_mesh
    coarse_mesh = full_hom_core
